    ...
    <div style={{maxWidth:'800px',margin:'0 auto'}}>
      {cats.map(cat => (<div key={cat.id} style={{marginBottom:'8px',border:'1px solid var(--border)',borderRadius:'12px',overflow:'hidden'}}>
        ...
        {openId===cat.id && (<div style={{padding:'0 24px 24px'}}>
          {(cat.items||[]).map((item,i) => (<div key={i} style={{display:'flex',justifyContent:'space-between',padding:'12px 0',borderBottom:'1px solid var(--border)'}}><div><div style={{fontWeight:600}}>{item.name}</div></div>{item.price && <div style={{color:'var(--primary)',fontWeight:600}}>{item.price}</div>}</div>))}
        </div>)}
      </div>))}
    </div>
  </section>);
