...
export function ServicesSection({ vendor }: { vendor: Vendor }) {
  ...
  return (<section id="services" className="section" style={{padding:'100px 40px'}}>
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))',gap:'24px',maxWidth:'1100px',margin:'0 auto'}}>
      {services.map((s,i) => (<div key={i} className="card"><div style={{fontSize:'36px',marginBottom:'16px'}}>{s.icon}</div><div style={{fontSize:'18px',fontWeight:700,marginBottom:'8px'}}>{s.name}</div>{s.description && <p style={{color:'var(--text-muted)',fontSize:'14px'}}>{s.description}</p>}</div>))}
    </div>
  </section>);
}
