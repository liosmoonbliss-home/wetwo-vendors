...
export function Dashboard({ vendor, links, onViewPublic }: { vendor: Vendor; links: Record<string,string|null>; onViewPublic: () => void }) {
  ...
  return (<div style={{display:'flex',minHeight:'100vh',background:'var(--bg)'}}>
    ...
    <main style={{flex:1,padding:'32px',overflow:'auto'}}>
      ...
      {panel==='earnings' && (<div>
        <p style={{color:'var(--text-muted)',marginBottom:'24px'}}>Your current tier: <strong style={{color:'var(--primary)'}}>{vendor.account_status==='pro'?'Pro (30% pool · 10% commission)':vendor.account_status==='elite'?'Elite (40% pool · 20% commission)':'Free (20% pool)'}</strong></p>
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))',gap:'16px'}}>
          {[{n:'Free',p:'$0',r:'20% buyer pool',c:'var(--text-dim)'},{n:'Pro',p:'$97/mo',r:'30% pool · 10% commission',c:'var(--green,#22c55e)'},{n:'Elite',p:'$197/mo',r:'40% pool · 20% commission',c:'var(--primary)'}].map(t=>(<div key={t.n} className="card" style={{textAlign:'center'}}><div style={{fontWeight:700,color:t.c,marginBottom:'4px'}}>{t.n}</div><div style={{fontSize:'24px',fontWeight:700}}>{t.p}</div><div style={{color:'var(--text-muted)',fontSize:'13px'}}>{t.r}</div></div>))}
        </div></div>)}
      {panel==='perks' && (<div className="card"><div style={{fontSize:'36px',marginBottom:'12px'}}>🎁</div><h3 style={{fontSize:'20px',fontWeight:600,marginBottom:'8px'}}>Personal Cash Back</h3><p style={{color:'var(--text-muted)',lineHeight:1.7}}>As a WeTwo vendor, you earn cash back on your own purchases through our partner stores. Use the same shopping link you share with clients!</p></div>)}
